...
export class ProjectPortfolioService {
  ...
  private async assessStrategicAlignment(projectId: string): Promise<number> {
    ...
    const strategicCriteria = [
      ...
      { criterion: 'Technology Innovation', weight: 0.2, score: 75 },
      { criterion: 'Competitive Advantage', weight: 0.2, score: 80 },
      { criterion: 'Customer Value', weight: 0.2, score: 90 },
      { criterion: 'Organizational Fit', weight: 0.15, score: 70 },
    ];

    const weightedScore = strategicCriteria.reduce(
  ...
  private async evaluateProjectRisk(projectId: string): Promise<any> {
    ...
    const overallRisk =
      riskCategories.reduce((sum, risk) => sum + risk.probability * risk.impact, 0) /
  ...
  private async assessResourceAvailability(projectId: string): Promise<any> {
    ...
    const averageScore =
      resourceRequirements.reduce((sum, req) => sum + req.score, 0) / resourceRequirements.length;
    ...
    return {
      ...
      constraints: resourceRequirements
        .filter((req) => req.score < 100)
        .map((req) => `${req.skill}: Need ${req.required}, have ${req.available}`),
    };
  }
  ...
  async prioritizeProjects(
    ...
    return {
      ...
      prioritizedProjects,
      selectedProjects,
      budgetUtilization: selectedProjects.reduce(
        (sum: number, p: any) => sum + p.estimatedBudget,
        0
      ),
  ...
  private optimizeProjectSelection(projects: any[], constraints: any): any[] {
    ...
    for (const project of projects) {
      ...
      if (remainingBudget >= estimatedBudget && selected.length < 5) {
        // Max 5 projects
        selected.push({
          ...project,
          estimatedBudget,
  ...
  private analyzePortfolioBalance(selectedProjects: any[]): any {
    const riskDistribution = {
      low: selectedProjects.filter((p) => p.details.riskAssessment.overallRiskLevel === 'LOW')
        .length,
      medium: selectedProjects.filter((p) => p.details.riskAssessment.overallRiskLevel === 'MEDIUM')
        .length,
      high: selectedProjects.filter((p) => p.details.riskAssessment.overallRiskLevel === 'HIGH')
        .length,
    };

    const averageROI =
      selectedProjects.reduce((sum, p) => sum + (p.details?.financialAnalysis?.roi || 0), 0) /
  ...
  private calculatePortfolioMetrics(evaluations: any[]): any {
    const totalValue = evaluations.reduce(
      (sum, e) => sum + (e.details?.financialAnalysis?.initialInvestment || 0),
      0
    );
    const averageScore =
    ...
    return {
      ...
      strategicAlignment: Math.round(
        evaluations.reduce((sum, e) => sum + e.scores.strategic, 0) / evaluations.length
      ),
    };
  }
