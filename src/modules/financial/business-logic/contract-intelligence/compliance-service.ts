...
import { ComplianceStatus, RegulationCompliance, CertificationStatus } from './types';
...
export class ComplianceService {
  ...
  async checkCertificationAlerts(): Promise<{
    ...
    const renewalReminders = [
      ...expiringSoon.map(
        (cert) => `${cert.certificationName} expires soon on ${cert.expirationDate?.toDateString()}`
      ),
      ...expired.map(
        (cert) => `${cert.certificationName} expired on ${cert.expirationDate?.toDateString()}`
      ),
    ];
