...
export class ShopFloorControlService {
  ...
  async startOperation(
    ...
    console.log(`Starting operation ${operationId} on workstation ${workstationId} by operator ${operatorId}`);
  ...
  async generateProductionMetrics(
    ...
    console.log(`Generating production metrics for ${workCenterCode} on ${date.toDateString()} ${shift} shift`);
  ...
  async reportShopFloorException(
    ...
    console.log(`Reported shop floor exception: ${exception.exceptionType} at ${exception.workCenterCode}`);
    return exception;
  }
