...
function renderTechnicianList(technicians) {
  ...
  container.innerHTML = sampleTechnicians
    .map(
      (tech) => `
...
function renderPendingQueue(dispatches) {
  ...
  container.innerHTML = sampleDispatches
    .map(
      (dispatch) => `
...
function renderMessages(messages) {
  ...
  container.innerHTML = sampleMessages
    .map(
      (msg) => `
...
function renderAlerts(alerts) {
  ...
  const sampleAlerts = alerts.length
    ? alerts
    : [
  ...
  container.innerHTML = sampleAlerts
    .map(
      (alert) => `
...
function renderNotifications(notifications) {
  ...
  const sampleNotifications = notifications.length
    ? notifications
    : [
        {
          id: 1,
  ...
  container.innerHTML = sampleNotifications
    .map(
      (notif) => `
