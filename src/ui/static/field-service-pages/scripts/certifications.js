...
async function loadCertificationsData() {
  try {
    ...
    const response = await fetch(
      `/api/field-service/certifications?${new URLSearchParams(filters)}`
    );
    if (response.ok) {
...
function addCertification() {
  ...
  if (
    !certificationData.technicianId ||
...
function scheduleTraining() {
  const attendees = Array.from(
    document.querySelectorAll('.attendees-selection input[type="checkbox"]:checked')
  ...
  if (!trainingData.title || !trainingData.date || !trainingData.time || attendees.length === 0) {
    showNotification(
      'Please fill in all required fields and select at least one attendee',
      'error'
    );
    return;
  }
