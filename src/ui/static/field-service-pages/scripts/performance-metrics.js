...
async function loadPerformanceData() {
    try {
        ...
        const response = await fetch(`/api/field-service/performance/metrics?${new URLSearchParams(filters)}`);
        if (response.ok) {
...
function updateTrendChart(metric) {
    ...
    if (dataPoints && trendData[metric]) {
        dataPoints.innerHTML = trendData[metric].map(point => 
...
function updateComparisonChart(metric) {
    ...
    if (barsContainer && comparisonData[metric]) {
        barsContainer.innerHTML = comparisonData[metric].map(item => `
            <div class="bar-item">
