...
async function loadScheduleData() {
    try {
        ...
        const response = await fetch(`/api/field-service/technicians/${technicianId}/schedule?week=${week}`);
        if (response.ok) {
    ...
    fetch('/api/field-service/work-orders/unassigned')
        ...
        .then(data => {
            ...
            if (select && data.success) {
                select.innerHTML = data.data.map(wo => 
                    `<option value="${wo.id}">${wo.id} - ${wo.title} (${wo.customer})</option>`
                ).join('');
            }
        })
        .catch(error => {
