...
async function loadServiceHistoryData() {
    try {
        ...
        const response = await fetch(`/api/field-service/customers/service-history?${new URLSearchParams(filters)}`);
        if (response.ok) {
...
function updateTrendChart(metric) {
    ...
    const trendData = {
        count: ['Jan: 24 services', 'Feb: 31 services', 'Mar: 28 services', 'Apr: 35 services', 'May: 29 services'],
    ...
    if (trendElement && trendData[selectedMetric]) {
        trendElement.innerHTML = trendData[selectedMetric].map(point => 
            `<div class="trend-point">${point}</div>`
        ).join('');
    }
...
function sortHistoryTable(sortBy) {
    ...
    rows.sort((a, b) => {
        ...
        switch (sortBy) {
            ...
            case 'value':
                aValue = parseFloat(a.querySelector('.service-value')?.textContent.replace(/[$,]/g, '') || '0');
                bValue = parseFloat(b.querySelector('.service-value')?.textContent.replace(/[$,]/g, '') || '0');
