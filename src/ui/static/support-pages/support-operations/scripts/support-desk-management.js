...
class SupportDeskManager {
  ...
  async loadAgents() {
    try {
      const response = await window.SupportPageManager.apiRequest(
        '/api/v1/support/support-operations/agents'
      );
  ...
  async loadQueues() {
    try {
      const response = await window.SupportPageManager.apiRequest(
        '/api/v1/support/support-operations/queues'
      );
  ...
  initializeMetrics() {
    ...
    this.metrics.set(
      'activeAgents',
      Array.from(this.agents.values()).filter((a) => a.status === 'available').length
    );
    this.metrics.set('totalQueues', this.queues.size);
    this.metrics.set(
      'totalTickets',
      Array.from(this.queues.values()).reduce((sum, q) => sum + q.currentSize, 0)
    );
