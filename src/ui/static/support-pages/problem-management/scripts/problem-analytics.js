/**
 * Problem Analytics JavaScript Module
 * Handles advanced problem analytics with trend analysis, cost impact assessment, and prevention effectiveness metrics
 */

class ProblemAnalyticsManager {
    constructor() {
        this.data = new Map();
        this.config = {};
    ...
    async init() {
        try {
            ...
            console.log('Problem Analytics Manager initialized successfully');
        } catch (error) {
            console.error('Problem Analytics Manager initialization failed:', error);
        }
    }
    ...
    async loadConfiguration() {
        try {
            ...
            console.warn('Using default configuration for Problem Analytics');
            this.config = {
                updateInterval: 30000,
                maxRecords: 1000,
                autoRefresh: true
    ...
    async loadData() {
        try {
            const response = await window.SupportPageManager.apiRequest('/api/v1/support/problem-management/problem-analytics');
            ...
            console.error('Failed to load Problem Analytics data:', error);
    ...
    async testIntegration() {
        try {
            window.SupportPageManager.showNotification('Testing Problem Analytics integration...', 'info');
            
            const tests = [
                { name: 'Problem Analytics API', endpoint: '/api/v1/support/problem-management/problem-analytics' },
                { name: 'Config API', endpoint: '/api/v1/support/problem-management/config' },
                { name: 'Analytics API', endpoint: '/api/v1/support/problem-management/problem-analytics/analytics' }
            ];
    ...
    async createRecord() {
        try {
            window.SupportPageManager.showNotification('Creating new problem analytics record...', 'info');
            // Implementation for creating new records
            console.log('Create record functionality for Problem Analytics');
        } catch (error) {
    ...
    async importData() {
        try {
            window.SupportPageManager.showNotification('Importing problem analytics data...', 'info');
            // Implementation for importing data
            console.log('Import data functionality for Problem Analytics');
        } catch (error) {
    ...
    async viewAnalytics() {
        try {
            window.SupportPageManager.showNotification('Loading problem analytics analytics...', 'info');
            // Implementation for viewing analytics
            console.log('View analytics functionality for Problem Analytics');
        } catch (error) {
    ...
    async exportReport() {
        try {
            window.SupportPageManager.showNotification('Generating problem analytics report...', 'info');
            
            const reportData = {
                title: 'Problem Analytics Report',
    ...
    async viewData() {
        try {
            console.log('View data functionality for Problem Analytics');
    ...
    async configure() {
        try {
            console.log('Configure functionality for Problem Analytics');
    ...
    async exportData() {
        try {
            window.SupportPageManager.showNotification('Exporting problem analytics data...', 'info');
            
            const exportData = {
                title: 'Problem Analytics Data Export',
                exported: new Date().toISOString(),
                data: Array.from(this.data.values()),
                config: this.config
            };

            this.downloadJSON(exportData, 'problem-analytics-data.json');
    ...
    handleRealTimeUpdate(data) {
        console.log('Real-time update received for Problem Analytics:', data);
...
document.addEventListener('DOMContentLoaded', () => {
    window.problemanalyticsManager = new ProblemAnalyticsManager();
});