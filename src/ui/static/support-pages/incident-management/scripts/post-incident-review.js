...
class PostIncidentReviewManager {
  ...
  async loadData() {
    try {
      const response = await window.SupportPageManager.apiRequest(
        '/api/v1/support/incident-management/post-incident-review'
      );
  ...
  async testIntegration() {
    try {
      window.SupportPageManager.showNotification(
        'Testing Post-Incident Review integration...',
        'info'
      );

      const tests = [
        {
          name: 'Post-Incident Review API',
          endpoint: '/api/v1/support/incident-management/post-incident-review',
        },
        { name: 'Config API', endpoint: '/api/v1/support/incident-management/config' },
        {
          name: 'Analytics API',
          endpoint: '/api/v1/support/incident-management/post-incident-review/analytics',
        },
      ];
  ...
  async createRecord() {
    try {
      window.SupportPageManager.showNotification(
        'Creating new post-incident review record...',
        'info'
      );
  ...
  async viewAnalytics() {
    try {
      window.SupportPageManager.showNotification(
        'Loading post-incident review analytics...',
        'info'
      );
  ...
  async exportReport() {
    try {
      window.SupportPageManager.showNotification(
        'Generating post-incident review report...',
        'info'
      );
