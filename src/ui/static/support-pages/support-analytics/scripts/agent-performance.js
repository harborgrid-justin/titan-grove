/**
 * Agent Performance JavaScript Module
 * Handles individual and team performance analytics with productivity metrics, quality scores, and development insights
 */
...
class AgentPerformanceManager {
    ...
    async init() {
        try {
            ...
            console.log('Agent Performance Manager initialized successfully');
        } catch (error) {
            console.error('Agent Performance Manager initialization failed:', error);
        }
    }
    ...
    async loadConfiguration() {
        try {
            ...
            console.warn('Using default configuration for Agent Performance');
            this.config = {
                updateInterval: 30000,
                maxRecords: 1000,
                autoRefresh: true
    ...
    async loadData() {
        try {
            const response = await window.SupportPageManager.apiRequest('/api/v1/support/support-analytics/agent-performance');
            ...
            console.error('Failed to load Agent Performance data:', error);
    ...
    async testIntegration() {
        try {
            window.SupportPageManager.showNotification('Testing Agent Performance integration...', 'info');
            
            const tests = [
                { name: 'Agent Performance API', endpoint: '/api/v1/support/support-analytics/agent-performance' },
                { name: 'Config API', endpoint: '/api/v1/support/support-analytics/config' },
                { name: 'Analytics API', endpoint: '/api/v1/support/support-analytics/agent-performance/analytics' }
            ];
    ...
    async createRecord() {
        try {
            window.SupportPageManager.showNotification('Creating new agent performance record...', 'info');
            // Implementation for creating new records
            console.log('Create record functionality for Agent Performance');
        } catch (error) {
    ...
    async importData() {
        try {
            window.SupportPageManager.showNotification('Importing agent performance data...', 'info');
            // Implementation for importing data
            console.log('Import data functionality for Agent Performance');
        } catch (error) {
    ...
    async viewAnalytics() {
        try {
            window.SupportPageManager.showNotification('Loading agent performance analytics...', 'info');
            // Implementation for viewing analytics
            console.log('View analytics functionality for Agent Performance');
        } catch (error) {
    ...
    async exportReport() {
        try {
            window.SupportPageManager.showNotification('Generating agent performance report...', 'info');
    ...
    async viewData() {
        try {
            console.log('View data functionality for Agent Performance');
    ...
    async configure() {
        try {
            console.log('Configure functionality for Agent Performance');
    ...
    async exportData() {
        try {
            window.SupportPageManager.showNotification('Exporting agent performance data...', 'info');
            
            const exportData = {
                title: 'Agent Performance Data Export',
                exported: new Date().toISOString(),
                data: Array.from(this.data.values()),
                config: this.config
            };

            this.downloadJSON(exportData, 'agent-performance-data.json');
    ...
    handleRealTimeUpdate(data) {
        console.log('Real-time update received for Agent Performance:', data);
...
document.addEventListener('DOMContentLoaded', () => {
    window.agentperformanceManager = new AgentPerformanceManager();
});