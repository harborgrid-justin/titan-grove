...
function handleriskMonitoringAction() {
    ...
    if (actionBtn) {
        ...
        setTimeout(() => {
            ...
            window.commandCenterPages.showNotification('Emergency Risk Monitoring action completed successfully', 'success');
        }, 2000);
    }
}
...
function loadriskMonitoringData() {
    ...
    if (window.commandCenterPages) {
        window.commandCenterPages.loadPageData('risk-monitoring').then(data => {
