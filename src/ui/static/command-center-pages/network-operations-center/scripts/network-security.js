...
function handlenetworkSecurityAction() {
    ...
    if (actionBtn) {
        ...
        setTimeout(() => {
            ...
            window.commandCenterPages.showNotification('Network Security Operations action completed successfully', 'success');
        }, 2000);
    }
}
...
function loadnetworkSecurityData() {
    ...
    if (window.commandCenterPages) {
        window.commandCenterPages.loadPageData('network-security').then(data => {
