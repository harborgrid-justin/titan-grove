...
function handlenetworkMonitoringAction() {
    ...
    if (actionBtn) {
        ...
        setTimeout(() => {
            ...
            window.commandCenterPages.showNotification('Network Performance Monitoring action completed successfully', 'success');
        }, 2000);
    }
}
...
function loadnetworkMonitoringData() {
    ...
    if (window.commandCenterPages) {
        window.commandCenterPages.loadPageData('network-monitoring').then(data => {
