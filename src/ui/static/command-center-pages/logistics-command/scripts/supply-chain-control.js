...
function handlesupplyChainControlAction() {
    ...
    if (actionBtn) {
        ...
        setTimeout(() => {
            ...
            window.commandCenterPages.showNotification('Supply Chain Control Tower action completed successfully', 'success');
        }, 2000);
    }
}
...
function loadsupplyChainControlData() {
    ...
    if (window.commandCenterPages) {
        window.commandCenterPages.loadPageData('supply-chain-control').then(data => {
