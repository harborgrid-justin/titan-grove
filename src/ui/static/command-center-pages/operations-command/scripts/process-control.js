...
function handleprocessControlAction() {
    ...
    if (actionBtn) {
        ...
        setTimeout(() => {
            ...
            window.commandCenterPages.showNotification('Process Control & Automation action completed successfully', 'success');
        }, 2000);
    }
}
...
function loadprocessControlData() {
    ...
    if (window.commandCenterPages) {
        window.commandCenterPages.loadPageData('process-control').then(data => {
