...
function handlecommandAnalyticsAction() {
    ...
    if (actionBtn) {
        ...
        setTimeout(() => {
            ...
            window.commandCenterPages.showNotification('Command Center Analytics action completed successfully', 'success');
        }, 2000);
    }
}
...
function loadcommandAnalyticsData() {
    ...
    if (window.commandCenterPages) {
        window.commandCenterPages.loadPageData('command-analytics').then(data => {
