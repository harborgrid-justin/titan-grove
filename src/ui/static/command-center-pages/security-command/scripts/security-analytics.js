...
function handlesecurityAnalyticsAction() {
    ...
    if (actionBtn) {
        ...
        setTimeout(() => {
            ...
            window.commandCenterPages.showNotification('Security Analytics & Intelligence action completed successfully', 'success');
        }, 2000);
    }
}
...
function loadsecurityAnalyticsData() {
    ...
    if (window.commandCenterPages) {
        window.commandCenterPages.loadPageData('security-analytics').then(data => {
