...
function handlequalityMonitoringAction() {
    ...
    if (actionBtn) {
        ...
        setTimeout(() => {
            ...
            window.commandCenterPages.showNotification('Quality Monitoring Center action completed successfully', 'success');
        }, 2000);
    }
}
...
function loadqualityMonitoringData() {
    ...
    if (window.commandCenterPages) {
        window.commandCenterPages.loadPageData('quality-monitoring').then(data => {
