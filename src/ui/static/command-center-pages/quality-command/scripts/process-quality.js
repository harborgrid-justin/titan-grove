...
function handleprocessQualityAction() {
    ...
    if (actionBtn) {
        ...
        setTimeout(() => {
            ...
            window.commandCenterPages.showNotification('Process Quality Management action completed successfully', 'success');
        }, 2000);
    }
}
...
function loadprocessQualityData() {
    ...
    if (window.commandCenterPages) {
        window.commandCenterPages.loadPageData('process-quality').then(data => {
