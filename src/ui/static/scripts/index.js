...
class TitanGroveMainEngine {
    ...
    createKPICard(container, data, kpiKey) {
        ...
        const statusClass = data.value >= (data.target * 0.9) ? 'success' : 
                           data.value >= (data.target * 0.75) ? 'warning' : 'error';
    ...
    updateModuleStatusDisplay() {
        ...
        const sortedModules = Array.from(this.activeModules.keys())
            ...
            .sort((a, b) => (this.moduleConfigs[a]?.priority || 999) - (this.moduleConfigs[b]?.priority || 999));
    ...
    generateSearchSuggestions(searchTerm) {
        ...
        const features = [
            { name: 'Financial Reports', module: 'financial', description: 'Generate financial reports and analytics' },
            { name: 'Work Orders', module: 'field-service', description: 'Manage field service work orders' },
            { name: 'Asset Maintenance', module: 'maintenance', description: 'Schedule and track asset maintenance' },
            { name: 'Manufacturing Orders', module: 'manufacturing', description: 'Manage production and manufacturing' }
        ];

        features.forEach(feature => {
            if (feature.name.toLowerCase().includes(term) || feature.description.toLowerCase().includes(term)) {
                suggestions.push({
                    type: 'feature',
                    title: feature.name,
    ...
    processModuleHealthUpdate(moduleHealthData) {
        ...
        moduleHealthData.forEach(moduleHealth => {
            ...
            if (currentModule && (currentModule.health !== moduleHealth.health || currentModule.status !== moduleHealth.status)) {
    ...
    getModuleDisplayName(moduleId) {
        ...
        return displayNames[moduleId] || moduleId.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    }
