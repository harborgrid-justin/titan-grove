...
class HRManagementSystem {
  ...
  async init() {
    ...
    try {
      ...
      console.log(
        `✅ HR Management System initialized in ${this.performanceMetrics.loadTime.toFixed(2)}ms`
      );
  ...
  async createPayrollExpensesChart() {
    ...
    const grossPayData = [
      2350000, 2380000, 2410000, 2390000, 2420000, 2450000, 2480000, 2460000, 2490000, 2510000,
      2530000, 2400000,
    ];
    const netPayData = [
      1762500, 1785000, 1807500, 1792500, 1815000, 1837500, 1860000, 1845000, 1867500, 1882500,
      1897500, 1800000,
    ];
    const benefitsData = [
      517500, 524000, 530750, 526250, 533000, 539250, 546000, 541000, 548250, 552750, 557250,
      528000,
    ];
  ...
  switchSection(sectionId) {
    ...
    const targetSection = document.getElementById(
      sectionId === 'dashboard' ? 'hr-dashboard' : `${sectionId}-management`
    );
    if (targetSection) {
  ...
  updateKPICards() {
    ...
    const kpiUpdates = {
      employees: {
        ...
        fullTime: this.employees.filter(
          (emp) => emp.status === 'active' && emp.position.includes('Senior')
        ).length,
        partTime: this.employees.filter(
          (emp) => emp.status === 'active' && !emp.position.includes('Senior')
        ).length,
      },
      payroll: {
  ...
  applyFilters() {
    ...
    this.filteredEmployees = this.employees.filter((employee) => {
      ...
      const locationMatch =
        !locationFilter || employee.location.toLowerCase().includes(locationFilter.toLowerCase());
