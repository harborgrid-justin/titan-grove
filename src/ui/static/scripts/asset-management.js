...
class TitanAssetManagementSystem {
        ...
        return [
            {
                ...
                description: 'Complete quarterly maintenance including lubrication, calibration, and safety checks',
                priority: 'Medium',
                status: 'In Progress',
                type: 'Preventive',
    ...
    updateOverviewDashboard() {
        ...
        const criticalAssets = Array.from(this.assets.values()).filter(a => a.criticality === 'Critical').length;
        const pendingMaintenance = Array.from(this.workOrders.values()).filter(wo => wo.status === 'Scheduled').length;
        ...
        if (operationalEl) operationalEl.textContent = `${Math.round((operationalAssets / totalAssets) * 100)}%`;
    ...
    updateMaintenanceOverview() {
        ...
        upcomingContainer.innerHTML = upcomingMaintenance.map(asset => `
    ...
    scheduleMaintenanceNow(assetId) {
        ...
        const workOrder = {
            id: 'wo-' + Date.now(),
            assetId: assetId,
            orderNumber: 'WO-' + new Date().getFullYear() + '-' + String(this.workOrders.size + 1).padStart(3, '0'),
    ...
    handleAssetSearch(query) {
        ...
        const filtered = Array.from(this.assets.values()).filter(asset =>
    ...
    generateMaintenanceCostReport() {
        ...
        const totalCost = Array.from(this.assets.values()).reduce((sum, asset) => sum + asset.maintenanceCost, 0);
