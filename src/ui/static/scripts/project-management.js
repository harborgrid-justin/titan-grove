...
class TitanProjectManagementSuite {
  ...
  calculateOverviewMetrics() {
    ...
    const totalCapacity = Array.from(this.resources.values()).reduce(
      (sum, resource) => sum + resource.maxCapacity,
      0
    );
    const totalWorkload = Array.from(this.resources.values()).reduce(
      (sum, resource) => sum + (resource.currentWorkload * resource.maxCapacity) / 100,
      0
    );
        ...
        datasets: [
          {
            ...
            backgroundColor: ['#6b7280', '#3b82f6', '#f59e0b', '#10b981', '#9ca3af'],
          },
        ],
      },
      options: {
  ...
  checkUpcomingDeadlines() {
    ...
    const upcomingTasks = Array.from(this.tasks.values()).filter((task) => {
