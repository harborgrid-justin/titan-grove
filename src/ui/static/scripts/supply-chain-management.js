...
class SupplyChainManagementSystem {
    ...
    async loadSupplyChainData() {
        ...
        console.log(`✅ Loaded ${this.inventory.length} inventory items, ${this.suppliers.length} suppliers`);
    }
    ...
    generateSampleInventory() {
        const categories = ['Electronics', 'Raw Materials', 'Components', 'Packaging', 'Tools', 'Chemicals'];
        const suppliers = ['TechSupply Corp', 'Global Manufacturing', 'Quality Components', 'Precision Parts', 'Industrial Supply'];
    ...
    async createSupplierPerformanceChart() {
        ...
        const suppliers = ['TechSupply Corp', 'Global Mfg', 'Quality Comp', 'Precision Parts', 'Industrial Supply'];
    ...
    calculateSupplyChainMetrics() {
        const inventoryValue = this.inventory.reduce((sum, item) => sum + (item.currentStock * item.unitCost), 0);
    ...
    switchSection(sectionId) {
        ...
        const targetSection = document.getElementById(sectionId === 'dashboard' ? 'scm-dashboard' : `${sectionId}-management`);
        if (targetSection) {
