...
class ServiceCommandCenterEngine {
  ...
  async initializeServices() {
    ...
    this.serviceCommandCenterService = {
      ...
      executeServiceWorkflow: async (workflowId, triggerId, context) =>
        this.executeWorkflow(workflowId, triggerId, context),
      updateResourceStatus: async (resourceId, status) =>
        this.updateResourceStatus(resourceId, status),
  ...
  async optimizeDispatch(commandCenterId, criteria) {
    ...
    const pendingOrders = Array.from(this.workOrders.values()).filter(
  ...
  updateKPIDisplay() {
    ...
    if (avgResponseTimeEl)
      avgResponseTimeEl.textContent = (metrics.avgResponseTime || 0).toFixed(1);
    ...
    if (customerSatisfactionEl)
      customerSatisfactionEl.textContent = (metrics.customerSatisfaction || 0).toFixed(1);
    ...
    if (activeResourcesCountEl)
      activeResourcesCountEl.textContent = metrics.activeWorkOrders || metrics.activeResources || 0;
    if (emergencyAlertsCountEl)
      emergencyAlertsCountEl.textContent = metrics.emergencyTickets || metrics.emergencyAlerts || 0;
  ...
  initializeDispatchMap() {
    ...
    this.dispatchMap = L.map('dispatchMap').setView([40.7128, -74.006], 10);
  ...
  findOptimalResource(workOrder, availableResources, criteria) {
    const eligibleResources = availableResources.filter(
      (resource) =>
    ...
    switch (criteria.criteria) {
      case 'response-time':
        return eligibleResources.reduce((best, current) =>
          this.calculateResponseTime(workOrder, current) <
          this.calculateResponseTime(workOrder, best)
            ? current
            : best
        );
      case 'cost':
  ...
  updateEmergencyDisplay() {
    ...
    emergencyLog.innerHTML = recentEvents
      .map(
        (event) => `
  ...
  async loadAnalyticsFromAPI() {
    try {
      const response = await fetch(
        '/api/service-command/analytics?timeRange=7d&metrics=response_time,completion_rate,customer_satisfaction'
      );
      const result = await response.json();
  ...
  async loadWorkOrdersFromAPI() {
    try {
      const response = await fetch(
        '/api/field-service/work-orders?status=ASSIGNED,SCHEDULED,IN_PROGRESS'
      );
      const result = await response.json();
      ...
      if (result.success && result.data) {
        ...
        result.data.forEach((tech) => {
          const resource = {
            ...
            status: tech.status,
            skills: tech.skills,
            currentLocation: tech.location || { lat: 40.7128, lng: -74.006 },
            performanceMetrics: {
  ...
  calculateKPIMetrics() {
    ...
    return {
      ...
      completionRate:
        resources.length > 0
          ? resources.reduce((sum, r) => sum + r.performanceMetrics.completionRate, 0) /
            resources.length
          : 0,
      resourceUtilization:
        resources.length > 0
      ...
      customerSatisfaction:
        resources.length > 0
          ? resources.reduce((sum, r) => sum + r.performanceMetrics.averageRating, 0) /
            resources.length
          : 0,
      activeResources: resources.filter((r) => r.status === 'AVAILABLE' || r.status === 'ASSIGNED')
        .length,
  ...
  refreshWorkOrdersList() {
    ...
    workOrdersList.innerHTML = workOrders
      .map(
        (order) => `
    ...
    resultsContainer.innerHTML = `
            <div class="optimization-results">
                ...
                <div class="optimized-assignments">
                    ...
                    ${
                      optimization.optimizedAssignments
                        ?.map(
                          (assignment) => `
                        <div class="assignment-item">
