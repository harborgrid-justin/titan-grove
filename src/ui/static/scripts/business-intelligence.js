  ...
  createPerformanceMatrix() {
    ...
    departments.forEach((dept) => {
      ...
      metrics.forEach(() => {
        ...
        const color =
          score > 85 ? 'excellent' : score > 75 ? 'good' : score > 65 ? 'average' : 'poor';
  ...
  async generateRecommendations(scenario, constraints = {}) {
    ...
    if (scenario.type === 'revenue_optimization') {
      recommendations.push(
        ...this.generateRevenueOptimizationRecommendations(scenario, constraints)
      );
    }
    ...
    if (scenario.type === 'resource_optimization') {
      recommendations.push(
        ...this.generateResourceOptimizationRecommendations(scenario, constraints)
      );
    }
  ...
  async optimizeResourceAllocation(resources, demands, constraints = {}) {
    ...
    if (totalDemand <= totalResources) {
      ...
      sortedDemands.forEach((demand) => {
        const availableResource = resources.find((r) => r.type === demand.type && r.available > 0);
