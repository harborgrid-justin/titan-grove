...
class SpendAnalysisManager {
    ...
    async loadSpendData() {
        ...
        this.spendData = {
            ...
            supplierCount: 247,
            categories: [
                { name: 'Manufacturing', amount: 4500000, percentage: 36, previousYear: 4100000, suppliers: 45, avgOrderValue: 25500 },
                { name: 'Technology', amount: 3200000, percentage: 25.6, previousYear: 2950000, suppliers: 28, avgOrderValue: 35800 },
                { name: 'Services', amount: 2800000, percentage: 22.4, previousYear: 2750000, suppliers: 67, avgOrderValue: 18200 },
                { name: 'Materials', amount: 2000000, percentage: 16, previousYear: 2100000, suppliers: 107, avgOrderValue: 12800 }
            ],
            suppliers: [
                { name: 'Acme Manufacturing Inc.', category: 'Manufacturing', amount: 2500000, percentage: 20, trend: 8 },
                { name: 'Global Tech Solutions', category: 'Technology', amount: 1800000, percentage: 14.4, trend: 12 },
                { name: 'Industrial Parts Corp', category: 'Parts', amount: 1500000, percentage: 12, trend: -3 },
                { name: 'Quality Components Ltd', category: 'Components', amount: 1200000, percentage: 9.6, trend: 5 },
    ...
    updateKPIs() {
        ...
        const previousYearTotal = this.spendData.categories.reduce((sum, cat) => sum + cat.previousYear, 0);
        const yoyGrowth = ((this.spendData.totalSpend - previousYearTotal) / previousYearTotal * 100).toFixed(1);
        
        // Update KPI cards
        const kpiCards = document.querySelectorAll('.spend-kpi-card');
        if (kpiCards.length >= 4) {
            // Total Spend
            kpiCards[0].querySelector('.spend-kpi-value').textContent = `$${(this.spendData.totalSpend / 1000000).toFixed(1)}M`;
            kpiCards[0].querySelector('.spend-kpi-change').innerHTML = `<i class="fas fa-arrow-up"></i> ${yoyGrowth}% vs last year`;
            
            // Monthly Spend
            kpiCards[1].querySelector('.spend-kpi-value').textContent = `$${(this.spendData.monthlySpend / 1000000).toFixed(2)}M`;
            
            // Cost Savings
            kpiCards[2].querySelector('.spend-kpi-value').textContent = `$${(this.spendData.costSavings / 1000).toFixed(0)}K`;
            const savingsRate = (this.spendData.costSavings / this.spendData.targetSavings * 100).toFixed(1);
            kpiCards[2].querySelector('.spend-kpi-change').innerHTML = `<i class="fas fa-arrow-up"></i> ${savingsRate}% achieved`;
    ...
    showSupplierDetails(item) {
        ...
        const modal = this.createModal('Supplier Spend Details', `
