...
function handlerevenuerecognitionAction() {
    ...
    window.financialPages.showNotification('Revenue Recognition (ASC 606) configured successfully', 'success');
}

function executerevenuerecognition() {
    console.log('Revenue Recognition (ASC 606) execution started');
    window.financialPages.showNotification('Revenue Recognition (ASC 606) executed successfully', 'success');
}
...
function setupPageActions() {
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/financial/operations-control/revenue-recognition/export');
