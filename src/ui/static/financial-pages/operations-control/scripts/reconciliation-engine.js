...
function handlereconciliationengineAction() {
  ...
  window.financialPages.showNotification(
    'Reconciliation Engine configured successfully',
    'success'
  );
}
...
function setupPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/operations-control/reconciliation-engine/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/operations-control/reconciliation-engine/export'
        );
