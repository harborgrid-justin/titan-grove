...
function handlebepscomplianceAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'BEPS Compliance Management configured successfully',
      'success'
    );
  }
}
...
function executebepscompliance() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'BEPS Compliance Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/international-tax-mgmt/beps-compliance/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/international-tax-mgmt/beps-compliance/export'
        );
