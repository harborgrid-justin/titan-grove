...
function handletreatybenefitsAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Treaty Benefits Management configured successfully',
      'success'
    );
  }
}
...
function executetreatybenefits() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Treaty Benefits Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/international-tax-mgmt/treaty-benefits/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/international-tax-mgmt/treaty-benefits/export'
        );
