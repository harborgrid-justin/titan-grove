...
function handlecfcsubpartfAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'CFC & Subpart F Management configured successfully',
      'success'
    );
  }
}
...
function executecfcsubpartf() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'CFC & Subpart F Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/international-tax-mgmt/cfc-subpart-f/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/international-tax-mgmt/cfc-subpart-f/export'
        );
