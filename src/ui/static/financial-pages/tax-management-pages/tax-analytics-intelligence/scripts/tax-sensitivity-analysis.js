...
async function loadtaxsensitivityanalysisData() {
    try {
        const response = await fetch('/api/tax-management/tax-analytics-intelligence/tax-sensitivity-analysis');
        if (response.ok) {
...
function handletaxsensitivityanalysisAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Sensitivity Analysis configured successfully', 'success');
    }
}
...
function executetaxsensitivityanalysis() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Sensitivity Analysis executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-analytics-intelligence/tax-sensitivity-analysis/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-analytics-intelligence/tax-sensitivity-analysis/export');
