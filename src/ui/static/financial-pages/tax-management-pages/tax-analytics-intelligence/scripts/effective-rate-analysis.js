...
async function loadeffectiverateanalysisData() {
  try {
    const response = await fetch(
      '/api/tax-management/tax-analytics-intelligence/effective-rate-analysis'
    );
    if (response.ok) {
...
function handleeffectiverateanalysisAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Effective Tax Rate Analysis configured successfully',
      'success'
    );
  }
}
...
function executeeffectiverateanalysis() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Effective Tax Rate Analysis executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-analytics-intelligence/effective-rate-analysis/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-analytics-intelligence/effective-rate-analysis/export'
        );
