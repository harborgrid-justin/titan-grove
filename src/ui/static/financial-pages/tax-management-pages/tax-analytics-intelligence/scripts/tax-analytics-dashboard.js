...
async function loadtaxanalyticsdashboardData() {
  try {
    const response = await fetch(
      '/api/tax-management/tax-analytics-intelligence/tax-analytics-dashboard'
    );
    if (response.ok) {
...
function handletaxanalyticsdashboardAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Analytics Dashboard configured successfully',
      'success'
    );
  }
}
...
function executetaxanalyticsdashboard() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Analytics Dashboard executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-analytics-intelligence/tax-analytics-dashboard/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-analytics-intelligence/tax-analytics-dashboard/export'
        );
