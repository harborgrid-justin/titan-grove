...
async function loadvarianceanalysisData() {
  try {
    const response = await fetch(
      '/api/tax-management/tax-analytics-intelligence/variance-analysis'
    );
    if (response.ok) {
...
function handlevarianceanalysisAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Variance Analysis configured successfully',
      'success'
    );
  }
}
...
function executevarianceanalysis() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Variance Analysis executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-analytics-intelligence/variance-analysis/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-analytics-intelligence/variance-analysis/export'
        );
