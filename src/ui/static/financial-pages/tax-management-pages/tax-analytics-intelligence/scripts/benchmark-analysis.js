...
async function loadbenchmarkanalysisData() {
  try {
    const response = await fetch(
      '/api/tax-management/tax-analytics-intelligence/benchmark-analysis'
    );
    if (response.ok) {
...
function handlebenchmarkanalysisAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Benchmark Analysis configured successfully',
      'success'
    );
  }
}
...
function executebenchmarkanalysis() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Benchmark Analysis executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-analytics-intelligence/benchmark-analysis/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-analytics-intelligence/benchmark-analysis/export'
        );
