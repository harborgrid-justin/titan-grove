...
function handledocumentmgmtAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Document Management configured successfully',
      'success'
    );
  }
}
...
function executedocumentmgmt() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Document Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-audit-documentation/document-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-audit-documentation/document-mgmt/export'
        );
