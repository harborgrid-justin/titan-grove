...
function handleauditresponsemgmtAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Audit Response Management configured successfully',
      'success'
    );
  }
}
...
function executeauditresponsemgmt() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Audit Response Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-audit-documentation/audit-response-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-audit-documentation/audit-response-mgmt/export'
        );
