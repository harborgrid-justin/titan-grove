...
async function loadsupportingschedulesData() {
    try {
        const response = await fetch('/api/tax-management/tax-audit-documentation/supporting-schedules');
        if (response.ok) {
...
function handlesupportingschedulesAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Supporting Tax Schedules configured successfully', 'success');
    }
}
...
function executesupportingschedules() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Supporting Tax Schedules executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-audit-documentation/supporting-schedules/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-audit-documentation/supporting-schedules/export');
