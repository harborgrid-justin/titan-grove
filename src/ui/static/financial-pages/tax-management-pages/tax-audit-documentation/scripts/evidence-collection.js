...
function handleevidencecollectionAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Evidence Collection configured successfully',
      'success'
    );
  }
}
...
function executeevidencecollection() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Evidence Collection executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-audit-documentation/evidence-collection/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-audit-documentation/evidence-collection/export'
        );
