...
async function loadworkpaperautomationData() {
    try {
        const response = await fetch('/api/tax-management/tax-audit-documentation/workpaper-automation');
        if (response.ok) {
...
function handleworkpaperautomationAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Workpaper Automation configured successfully', 'success');
    }
}
...
function executeworkpaperautomation() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Workpaper Automation executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-audit-documentation/workpaper-automation/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-audit-documentation/workpaper-automation/export');
