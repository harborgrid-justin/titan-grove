...
function handleauditpreparationAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Audit Preparation configured successfully',
      'success'
    );
  }
}
...
function executeauditpreparation() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Audit Preparation executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-audit-documentation/audit-preparation/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-audit-documentation/audit-preparation/export'
        );
