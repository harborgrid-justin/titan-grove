...
function handleaudittrailmgmtAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Audit Trail Management configured successfully',
      'success'
    );
  }
}
...
function executeaudittrailmgmt() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Audit Trail Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-audit-documentation/audit-trail-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-audit-documentation/audit-trail-mgmt/export'
        );
