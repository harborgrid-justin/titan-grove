...
function handletaxruleengineAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Rule Configuration Engine configured successfully', 'success');
    }
}
...
function executetaxruleengine() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Rule Configuration Engine executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-configuration-rules/tax-rule-engine/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-configuration-rules/tax-rule-engine/export');
