...
function handleintegrationconfigAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Integration Configuration configured successfully', 'success');
    }
}
...
function executeintegrationconfig() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Integration Configuration executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-configuration-rules/integration-config/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-configuration-rules/integration-config/export');
