...
function handlejurisdictionmgmtAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Jurisdiction Management configured successfully',
      'success'
    );
  }
}
...
function executejurisdictionmgmt() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Jurisdiction Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-configuration-rules/jurisdiction-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-configuration-rules/jurisdiction-mgmt/export'
        );
