...
function handleworkflowconfigAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Workflow Configuration configured successfully', 'success');
    }
}
...
function executeworkflowconfig() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Workflow Configuration executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-configuration-rules/workflow-config/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-configuration-rules/workflow-config/export');
