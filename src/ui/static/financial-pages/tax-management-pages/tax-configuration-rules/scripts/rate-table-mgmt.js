...
function handleratetablemgmtAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Rate Table Management configured successfully',
      'success'
    );
  }
}
...
function executeratetablemgmt() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Rate Table Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-configuration-rules/rate-table-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-configuration-rules/rate-table-mgmt/export'
        );
