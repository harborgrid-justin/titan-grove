...
function handlevatmanagementAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'VAT Management System configured successfully',
      'success'
    );
  }
}
...
function executevatmanagement() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'VAT Management System executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-calculation-processing/vat-management/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-calculation-processing/vat-management/export'
        );
