...
async function loadadvancedtaxengineData() {
    try {
        const response = await fetch('/api/tax-management/tax-calculation-processing/advanced-tax-engine');
        if (response.ok) {
...
function handleadvancedtaxengineAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Advanced Tax Calculation Engine configured successfully', 'success');
    }
}
...
function executeadvancedtaxengine() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Advanced Tax Calculation Engine executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-calculation-processing/advanced-tax-engine/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-calculation-processing/advanced-tax-engine/export');
