...
async function loadpropertytaxmgmtData() {
    try {
        const response = await fetch('/api/tax-management/tax-calculation-processing/property-tax-mgmt');
        if (response.ok) {
...
function handlepropertytaxmgmtAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Property Tax Management configured successfully', 'success');
    }
}
...
function executepropertytaxmgmt() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Property Tax Management executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-calculation-processing/property-tax-mgmt/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-calculation-processing/property-tax-mgmt/export');
