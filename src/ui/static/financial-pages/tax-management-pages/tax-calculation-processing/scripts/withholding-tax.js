...
function handlewithholdingtaxAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Withholding Tax Management configured successfully',
      'success'
    );
  }
}
...
function executewithholdingtax() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Withholding Tax Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-calculation-processing/withholding-tax/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-calculation-processing/withholding-tax/export'
        );
