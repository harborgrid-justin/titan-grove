...
function handleexcisetaxmgmtAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Excise Tax Management configured successfully',
      'success'
    );
  }
}
...
function executeexcisetaxmgmt() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Excise Tax Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-calculation-processing/excise-tax-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-calculation-processing/excise-tax-mgmt/export'
        );
