...
function handleusetaxtrackingAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Use Tax Tracking & Accrual configured successfully',
      'success'
    );
  }
}
...
function executeusetaxtracking() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Use Tax Tracking & Accrual executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-calculation-processing/use-tax-tracking/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-calculation-processing/use-tax-tracking/export'
        );
