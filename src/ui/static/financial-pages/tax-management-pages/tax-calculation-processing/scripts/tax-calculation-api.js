...
async function loadtaxcalculationapiData() {
    try {
        const response = await fetch('/api/tax-management/tax-calculation-processing/tax-calculation-api');
        if (response.ok) {
...
function handletaxcalculationapiAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Calculation API Hub configured successfully', 'success');
    }
}
...
function executetaxcalculationapi() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Calculation API Hub executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-calculation-processing/tax-calculation-api/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-calculation-processing/tax-calculation-api/export');
