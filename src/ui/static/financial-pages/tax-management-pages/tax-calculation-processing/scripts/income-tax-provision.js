...
async function loadincometaxprovisionData() {
  try {
    const response = await fetch(
      '/api/tax-management/tax-calculation-processing/income-tax-provision'
    );
    if (response.ok) {
...
function handleincometaxprovisionAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Income Tax Provision Engine configured successfully',
      'success'
    );
  }
}
...
function executeincometaxprovision() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Income Tax Provision Engine executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-calculation-processing/income-tax-provision/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-calculation-processing/income-tax-provision/export'
        );
