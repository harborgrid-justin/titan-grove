...
async function loadtaxreturnautomationData() {
    try {
        const response = await fetch('/api/tax-management/tax-compliance-reporting/tax-return-automation');
        if (response.ok) {
...
function handletaxreturnautomationAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Return Automation configured successfully', 'success');
    }
}
...
function executetaxreturnautomation() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Return Automation executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-compliance-reporting/tax-return-automation/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-compliance-reporting/tax-return-automation/export');
