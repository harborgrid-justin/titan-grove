...
function handletaxcalendarmgmtAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Calendar Management configured successfully',
      'success'
    );
  }
}
...
function executetaxcalendarmgmt() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Calendar Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-compliance-reporting/tax-calendar-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-compliance-reporting/tax-calendar-mgmt/export'
        );
