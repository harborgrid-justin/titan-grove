...
async function loadtaxdisclosuremgmtData() {
  try {
    const response = await fetch(
      '/api/tax-management/tax-compliance-reporting/tax-disclosure-mgmt'
    );
    if (response.ok) {
...
function handletaxdisclosuremgmtAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Disclosure Management configured successfully',
      'success'
    );
  }
}
...
function executetaxdisclosuremgmt() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Disclosure Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-compliance-reporting/tax-disclosure-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-compliance-reporting/tax-disclosure-mgmt/export'
        );
