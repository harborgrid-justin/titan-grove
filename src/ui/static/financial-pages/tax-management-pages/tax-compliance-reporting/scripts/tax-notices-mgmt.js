...
function handletaxnoticesmgmtAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Notices Management configured successfully',
      'success'
    );
  }
}
...
function executetaxnoticesmgmt() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Notices Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-compliance-reporting/tax-notices-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-compliance-reporting/tax-notices-mgmt/export'
        );
