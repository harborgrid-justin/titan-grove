...
async function loadauditdocumentationData() {
    try {
        const response = await fetch('/api/tax-management/tax-compliance-reporting/audit-documentation');
        if (response.ok) {
...
function handleauditdocumentationAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Audit Documentation configured successfully', 'success');
    }
}
...
function executeauditdocumentation() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Audit Documentation executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-compliance-reporting/audit-documentation/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-compliance-reporting/audit-documentation/export');
