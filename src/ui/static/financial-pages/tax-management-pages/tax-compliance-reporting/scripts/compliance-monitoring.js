...
async function loadcompliancemonitoringData() {
  try {
    const response = await fetch(
      '/api/tax-management/tax-compliance-reporting/compliance-monitoring'
    );
    if (response.ok) {
...
function handlecompliancemonitoringAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Compliance Monitoring configured successfully',
      'success'
    );
  }
}
...
function executecompliancemonitoring() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Tax Compliance Monitoring executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-compliance-reporting/compliance-monitoring/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-compliance-reporting/compliance-monitoring/export'
        );
