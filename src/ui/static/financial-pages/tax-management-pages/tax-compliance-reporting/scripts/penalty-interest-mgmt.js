...
async function loadpenaltyinterestmgmtData() {
  try {
    const response = await fetch(
      '/api/tax-management/tax-compliance-reporting/penalty-interest-mgmt'
    );
    if (response.ok) {
...
function handlepenaltyinterestmgmtAction() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Penalty & Interest Management configured successfully',
      'success'
    );
  }
}
...
function executepenaltyinterestmgmt() {
  ...
  if (window.financialPages) {
    window.financialPages.showNotification(
      'Penalty & Interest Management executed successfully',
      'success'
    );
  }
}
...
function setupTaxPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-compliance-reporting/penalty-interest-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/tax-management/tax-compliance-reporting/penalty-interest-mgmt/export'
        );
