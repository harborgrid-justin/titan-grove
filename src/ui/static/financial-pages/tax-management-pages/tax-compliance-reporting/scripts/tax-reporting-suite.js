...
async function loadtaxreportingsuiteData() {
    try {
        const response = await fetch('/api/tax-management/tax-compliance-reporting/tax-reporting-suite');
        if (response.ok) {
...
function handletaxreportingsuiteAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('Tax Reporting Suite configured successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-compliance-reporting/tax-reporting-suite/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-compliance-reporting/tax-reporting-suite/export');
