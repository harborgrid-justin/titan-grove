...
async function loadinternationalreportingData() {
    try {
        const response = await fetch('/api/tax-management/tax-compliance-reporting/international-reporting');
        if (response.ok) {
...
function handleinternationalreportingAction() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('International Tax Reporting configured successfully', 'success');
    }
}
...
function executeinternationalreporting() {
    ...
    if (window.financialPages) {
        window.financialPages.showNotification('International Tax Reporting executed successfully', 'success');
    }
}
...
function setupTaxPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-compliance-reporting/international-reporting/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/tax-management/tax-compliance-reporting/international-reporting/export');
