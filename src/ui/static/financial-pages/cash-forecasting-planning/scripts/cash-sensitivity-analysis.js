...
async function loadcashsensitivityanalysisData() {
  try {
    const response = await fetch(
      '/api/financial/cash-forecasting-planning/cash-sensitivity-analysis'
    );
    if (response.ok) {
...
function handlecashsensitivityanalysisAction() {
  ...
  window.financialPages.showNotification(
    'Cash Sensitivity Analysis configured successfully',
    'success'
  );
}
...
function executecashsensitivityanalysis() {
  ...
  window.financialPages.showNotification(
    'Cash Sensitivity Analysis executed successfully',
    'success'
  );
}
...
function setupPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/cash-forecasting-planning/cash-sensitivity-analysis/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/cash-forecasting-planning/cash-sensitivity-analysis/export'
        );
