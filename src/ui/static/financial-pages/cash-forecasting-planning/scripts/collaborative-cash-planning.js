...
async function loadcollaborativecashplanningData() {
  try {
    const response = await fetch(
      '/api/financial/cash-forecasting-planning/collaborative-cash-planning'
    );
    if (response.ok) {
...
function handlecollaborativecashplanningAction() {
  ...
  window.financialPages.showNotification(
    'Collaborative Cash Planning configured successfully',
    'success'
  );
}
...
function executecollaborativecashplanning() {
  ...
  window.financialPages.showNotification(
    'Collaborative Cash Planning executed successfully',
    'success'
  );
}
...
function setupPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/cash-forecasting-planning/collaborative-cash-planning/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/cash-forecasting-planning/collaborative-cash-planning/export'
        );
