...
async function loadcashbudgetintegrationData() {
  try {
    const response = await fetch(
      '/api/financial/cash-forecasting-planning/cash-budget-integration'
    );
    if (response.ok) {
...
function handlecashbudgetintegrationAction() {
  ...
  window.financialPages.showNotification(
    'Cash Budget Integration configured successfully',
    'success'
  );
}
...
function executecashbudgetintegration() {
  ...
  window.financialPages.showNotification(
    'Cash Budget Integration executed successfully',
    'success'
  );
}
...
function setupPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/cash-forecasting-planning/cash-budget-integration/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/cash-forecasting-planning/cash-budget-integration/export'
        );
