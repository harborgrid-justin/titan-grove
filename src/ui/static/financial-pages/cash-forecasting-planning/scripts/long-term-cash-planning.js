...
async function loadlongtermcashplanningData() {
  try {
    const response = await fetch(
      '/api/financial/cash-forecasting-planning/long-term-cash-planning'
    );
    if (response.ok) {
...
function handlelongtermcashplanningAction() {
  ...
  window.financialPages.showNotification(
    'Long-Term Cash Planning configured successfully',
    'success'
  );
}
...
function executelongtermcashplanning() {
  ...
  window.financialPages.showNotification(
    'Long-Term Cash Planning executed successfully',
    'success'
  );
}
...
function setupPageActions() {
  ...
  if (testBtn) {
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/cash-forecasting-planning/long-term-cash-planning/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/cash-forecasting-planning/long-term-cash-planning/export'
        );
