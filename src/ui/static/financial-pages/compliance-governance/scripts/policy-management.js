...
function handlepolicymanagementAction() {
    ...
    window.financialPages.showNotification('Financial Policy Management configured successfully', 'success');
}

function executepolicymanagement() {
    console.log('Financial Policy Management execution started');
    window.financialPages.showNotification('Financial Policy Management executed successfully', 'success');
}
...
function setupPageActions() {
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/financial/compliance-governance/policy-management/export');
