...
async function loadtreasurypositionmgmtData() {
    try {
        const response = await fetch('/api/financial/treasury-operations-advanced/treasury-position-mgmt');
        if (response.ok) {
...
function handletreasurypositionmgmtAction() {
    ...
    window.financialPages.showNotification('Treasury Position Management configured successfully', 'success');
}

function executetreasurypositionmgmt() {
    console.log('Treasury Position Management execution started');
    window.financialPages.showNotification('Treasury Position Management executed successfully', 'success');
}
...
function setupPageActions() {
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/financial/treasury-operations-advanced/treasury-position-mgmt/test');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', async function() {
            try {
                const response = await fetch('/api/financial/treasury-operations-advanced/treasury-position-mgmt/export');
