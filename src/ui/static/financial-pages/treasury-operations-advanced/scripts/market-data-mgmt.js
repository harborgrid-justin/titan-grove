...
function handlemarketdatamgmtAction() {
  ...
  window.financialPages.showNotification(
    'Market Data Management configured successfully',
    'success'
  );
}
...
function setupPageActions() {
    ...
    testBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/treasury-operations-advanced/market-data-mgmt/test'
        );
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/treasury-operations-advanced/market-data-mgmt/export'
        );
