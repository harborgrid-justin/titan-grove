...
function handlepaymentreconciliationAction() {
  ...
  window.financialPages.showNotification(
    'Payment Reconciliation Engine configured successfully',
    'success'
  );
}
...
function executepaymentreconciliation() {
  ...
  window.financialPages.showNotification(
    'Payment Reconciliation Engine executed successfully',
    'success'
  );
}
    ...
    exportBtn.addEventListener('click', async function () {
      try {
        const response = await fetch(
          '/api/financial/payment-systems/payment-reconciliation/export'
        );
