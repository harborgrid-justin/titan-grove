...
class CustomerProfilesManager {
        ...
        this.businessLogic = {
            segmentation: {
                rules: [
                    ...
                    { name: 'Mid-Market', criteria: { revenue: { min: 50000, max: 99999 }, employees: { min: 100, max: 499 } } },
    ...
    calculateMetrics() {
        ...
        return {
            ...
            'avg-customer-value': '$' + Math.round(customers.reduce((sum, c) => sum + c.revenue, 0) / customers.length).toLocaleString(),
            'retention-rate': '94.3%'
        };
    }
    ...
    handleSearch(query) {
        ...
        const filteredCustomers = Array.from(this.customers.values()).filter(customer => 
    ...
    convertToCSV(customers) {
        const headers = ['ID', 'Name', 'Email', 'Segment', 'Status', 'Revenue', 'Score', 'Industry', 'Employees'];
        const rows = customers.map(c => [
            c.id, c.name, c.email, c.segment, c.status, c.revenue, c.score, c.industry, c.employees
        ]);
