...
interface ProcedureManagementData {
  id: string;
  name: string;
...
const ProcedureManagement: React.FC = () => {
  const [data, setData] = useState<ProcedureManagementData[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<ProcedureManagementData | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [analytics, setAnalytics] = useState<any>(null);

  // Sample data specific to Procedure Management
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      
      // Simulate API call with healthcare-specific data
      const mockData: ProcedureManagementData[] = [
        {
          id: 'proceduremanagement-001',
          name: 'Sample Procedure Management Record 1',
          status: 'active',
          priority: 'high',
        ...
        {
          id: 'proceduremanagement-002',
          name: 'Sample Procedure Management Record 2',
          status: 'pending',
          priority: 'medium',
          ...
          category: 'Clinical Operations',
          description: 'Secondary record for medical procedure scheduling and tracking'
        },
        {
          id: 'proceduremanagement-003',
          name: 'Sample Procedure Management Record 3',
          status: 'completed',
          priority: 'low',
          ...
          category: 'Clinical Operations',
          description: 'Completed medical procedure scheduling and tracking'
        }
      ];
  ...
  const handleView = (item: ProcedureManagementData) => {
    // Implementation for viewing item details
    console.log('Viewing item:', item);
  };

  const handleEdit = (item: ProcedureManagementData) => {
    setEditingItem(item);
    setShowModal(true);
  };
  ...
  const handleSave = (formData: Partial<ProcedureManagementData>) => {
    if (editingItem) {
      ...
      // Add new item
      const newItem: ProcedureManagementData = {
        id: `proceduremanagement-${Date.now()}`,
        name: formData.name || 'New Procedure Management',
        ...
        category: 'Clinical Operations',
        description: formData.description || 'Medical procedure scheduling and tracking'
      };
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem' }}>
        <Loading description="Loading Procedure Management..." />
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '2rem' }}>
        ...
        <Column lg={16} md={8} sm={4}>
          <div style={{ marginBottom: '2rem' }}>
            ...
            <p style={{ color: '#6f6f6f', marginTop: '0.5rem' }}>
              Medical procedure scheduling and tracking
            </p>
          </div>
        </Column>
      </Grid>
        ...
        <Column lg={16} md={8} sm={4}>
          <DataTable
            ...
            render={({ rows, headers, getTableProps, getHeaderProps, getRowProps }) => (
              <TableContainer title="Procedure Management Management">
                <Table {...getTableProps()}>
                  <TableHead>
                    <TableRow>
                      {headers.map(header => (
      ...
      <Modal
        ...
        modalHeading={editingItem ? `Edit ${editingItem.name}` : 'Add New Procedure Management'}
        primaryButtonText="Save"
        secondaryButtonText="Cancel"
        onRequestSubmit={() => {
          // Get form data and save
          const formData = {
            name: 'Updated Procedure Management',
            status: 'active' as const,
            priority: 'medium' as const,
            assignedTo: 'Current User',
            description: 'Updated medical procedure scheduling and tracking'
          };
          handleSave(formData);
        }}
      >
