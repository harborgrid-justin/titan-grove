...
const CarePlanning: React.FC = () => {
  const [data, setData] = useState<CarePlanningData[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<CarePlanningData | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [analytics, setAnalytics] = useState<any>(null);

  // Sample data specific to Care Planning
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      
      // Simulate API call with healthcare-specific data
      const mockData: CarePlanningData[] = [
        {
          id: 'careplanning-001',
          name: 'Sample Care Planning Record 1',
          status: 'active',
          priority: 'high',
        ...
        {
          id: 'careplanning-002',
          name: 'Sample Care Planning Record 2',
          status: 'pending',
          priority: 'medium',
          ...
          category: 'Patient Management',
          description: 'Secondary record for individualized care plan development and management'
        },
        {
          id: 'careplanning-003',
          name: 'Sample Care Planning Record 3',
          status: 'completed',
          priority: 'low',
          ...
          category: 'Patient Management',
          description: 'Completed individualized care plan development and management'
        }
      ];
  ...
  const handleView = (item: CarePlanningData) => {
    // Implementation for viewing item details
    console.log('Viewing item:', item);
  };

  const handleEdit = (item: CarePlanningData) => {
    setEditingItem(item);
    setShowModal(true);
  };
  ...
  const handleSave = (formData: Partial<CarePlanningData>) => {
    if (editingItem) {
      ...
      // Add new item
      const newItem: CarePlanningData = {
        id: `careplanning-${Date.now()}`,
        name: formData.name || 'New Care Planning',
        ...
        category: 'Patient Management',
        description: formData.description || 'Individualized care plan development and management'
      };
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem' }}>
        <Loading description="Loading Care Planning..." />
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '2rem' }}>
        ...
        <Column lg={16} md={8} sm={4}>
          <div style={{ marginBottom: '2rem' }}>
            ...
            <p style={{ color: '#6f6f6f', marginTop: '0.5rem' }}>
              Individualized care plan development and management
            </p>
          </div>
        </Column>
      </Grid>
        ...
        <Column lg={16} md={8} sm={4}>
          <DataTable
            ...
            render={({ rows, headers, getTableProps, getHeaderProps, getRowProps }) => (
              <TableContainer title="Care Planning Management">
                <Table {...getTableProps()}>
                  <TableHead>
                    <TableRow>
                      {headers.map(header => (
      ...
      <Modal
        ...
        modalHeading={editingItem ? `Edit ${editingItem.name}` : 'Add New Care Planning'}
        primaryButtonText="Save"
        secondaryButtonText="Cancel"
        onRequestSubmit={() => {
          // Get form data and save
          const formData = {
            name: 'Updated Care Planning',
            status: 'active' as const,
            priority: 'medium' as const,
            assignedTo: 'Current User',
            description: 'Updated individualized care plan development and management'
          };
          handleSave(formData);
        }}
      >
