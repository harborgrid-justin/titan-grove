...
interface InsuranceManagementData {
  id: string;
  name: string;
...
const InsuranceManagement: React.FC = () => {
  const [data, setData] = useState<InsuranceManagementData[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<InsuranceManagementData | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [analytics, setAnalytics] = useState<any>(null);

  // Sample data specific to Insurance Management
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      
      // Simulate API call with healthcare-specific data
      const mockData: InsuranceManagementData[] = [
        {
          id: 'insurancemanagement-001',
          name: 'Sample Insurance Management Record 1',
          status: 'active',
          priority: 'high',
        ...
        {
          id: 'insurancemanagement-002',
          name: 'Sample Insurance Management Record 2',
          status: 'pending',
          priority: 'medium',
          ...
          category: 'Revenue Cycle',
          description: 'Secondary record for insurance verification and prior authorization'
        },
        {
          id: 'insurancemanagement-003',
          name: 'Sample Insurance Management Record 3',
          status: 'completed',
          priority: 'low',
          ...
          category: 'Revenue Cycle',
          description: 'Completed insurance verification and prior authorization'
        }
      ];
  ...
  const handleView = (item: InsuranceManagementData) => {
    // Implementation for viewing item details
    console.log('Viewing item:', item);
  };

  const handleEdit = (item: InsuranceManagementData) => {
    setEditingItem(item);
    setShowModal(true);
  };
  ...
  const handleSave = (formData: Partial<InsuranceManagementData>) => {
    if (editingItem) {
      ...
      // Add new item
      const newItem: InsuranceManagementData = {
        id: `insurancemanagement-${Date.now()}`,
        name: formData.name || 'New Insurance Management',
        ...
        category: 'Revenue Cycle',
        description: formData.description || 'Insurance verification and prior authorization'
      };
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem' }}>
        <Loading description="Loading Insurance Management..." />
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '2rem' }}>
        ...
        <Column lg={16} md={8} sm={4}>
          <DataTable
            ...
            render={({ rows, headers, getTableProps, getHeaderProps, getRowProps }) => (
              <TableContainer title="Insurance Management Management">
                <Table {...getTableProps()}>
                  <TableHead>
                    <TableRow>
                      {headers.map(header => (
      ...
      <Modal
        ...
        modalHeading={editingItem ? `Edit ${editingItem.name}` : 'Add New Insurance Management'}
        primaryButtonText="Save"
        secondaryButtonText="Cancel"
        onRequestSubmit={() => {
          // Get form data and save
          const formData = {
            name: 'Updated Insurance Management',
            status: 'active' as const,
            priority: 'medium' as const,
            assignedTo: 'Current User',
            description: 'Updated insurance verification and prior authorization'
          };
          handleSave(formData);
        }}
      >
