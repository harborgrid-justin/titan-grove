...
const ComplianceMonitoring: React.FC = () => {
  ...
  const [editingItem, setEditingItem] = useState<ComplianceMonitoringData | null>(null);
  ...
  const handleView = (item: ComplianceMonitoringData) => {
    // Implementation for viewing item details
    console.log('Viewing item:', item);
  };

  const handleEdit = (item: ComplianceMonitoringData) => {
    setEditingItem(item);
    setShowModal(true);
  };
  ...
  const handleSave = (formData: Partial<ComplianceMonitoringData>) => {
    if (editingItem) {
      ...
      <Modal
        ...
        onRequestSubmit={() => {
          ...
          const formData = {
            ...
            description: 'Updated real-time compliance monitoring and alerts'
          };
          handleSave(formData);
        }}
      >
