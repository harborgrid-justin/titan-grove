...
const PatientSupport: React.FC = () => {
  const [data, setData] = useState<PatientSupportData[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<PatientSupportData | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [analytics, setAnalytics] = useState<any>(null);

  // Sample data specific to Patient Support Services
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      
      // Simulate API call with healthcare-specific data
      const mockData: PatientSupportData[] = [
        {
          id: 'patientsupport-001',
          name: 'Sample Patient Support Services Record 1',
          status: 'active',
          priority: 'high',
        ...
        {
          id: 'patientsupport-002',
          name: 'Sample Patient Support Services Record 2',
          status: 'pending',
          priority: 'medium',
          ...
          category: 'Patient Management',
          description: 'Secondary record for comprehensive patient support and social services'
        },
        {
          id: 'patientsupport-003',
          name: 'Sample Patient Support Services Record 3',
          status: 'completed',
          priority: 'low',
          ...
          category: 'Patient Management',
          description: 'Completed comprehensive patient support and social services'
        }
      ];
  ...
  const handleView = (item: PatientSupportData) => {
    // Implementation for viewing item details
    console.log('Viewing item:', item);
  };

  const handleEdit = (item: PatientSupportData) => {
    setEditingItem(item);
    setShowModal(true);
  };
  ...
  const handleSave = (formData: Partial<PatientSupportData>) => {
    if (editingItem) {
      ...
      // Add new item
      const newItem: PatientSupportData = {
        id: `patientsupport-${Date.now()}`,
        name: formData.name || 'New Patient Support Services',
        ...
        category: 'Patient Management',
        description: formData.description || 'Comprehensive patient support and social services'
      };
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem' }}>
        <Loading description="Loading Patient Support Services..." />
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <Grid>
        <Column lg={16} md={8} sm={4}>
          <div style={{ marginBottom: '2rem' }}>
            <h1>Patient Support Services</h1>
            <p style={{ color: '#6f6f6f', marginTop: '0.5rem' }}>
              Comprehensive patient support and social services
            </p>
          </div>
        </Column>
      </Grid>
        ...
        <Column lg={16} md={8} sm={4}>
          <DataTable
            ...
            render={({ rows, headers, getTableProps, getHeaderProps, getRowProps }) => (
              <TableContainer title="Patient Support Services Management">
                <Table {...getTableProps()}>
                  <TableHead>
                    <TableRow>
                      {headers.map(header => (
      ...
      <Modal
        ...
        modalHeading={editingItem ? `Edit ${editingItem.name}` : 'Add New Patient Support Services'}
        primaryButtonText="Save"
        secondaryButtonText="Cancel"
        onRequestSubmit={() => {
          // Get form data and save
          const formData = {
            name: 'Updated Patient Support Services',
            status: 'active' as const,
            priority: 'medium' as const,
            assignedTo: 'Current User',
            description: 'Updated comprehensive patient support and social services'
          };
          handleSave(formData);
        }}
      >
