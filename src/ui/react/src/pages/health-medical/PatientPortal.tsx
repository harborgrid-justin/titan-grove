...
const PatientPortal: React.FC = () => {
  const [data, setData] = useState<PatientPortalData[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<PatientPortalData | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [analytics, setAnalytics] = useState<any>(null);

  // Sample data specific to Patient Portal
  useEffect(() => {
    const loadData = async () => {
      ...
      const mockData: PatientPortalData[] = [
        {
          id: 'patientportal-001',
          name: 'Sample Patient Portal Record 1',
          status: 'active',
          priority: 'high',
        ...
        {
          id: 'patientportal-002',
          name: 'Sample Patient Portal Record 2',
          status: 'pending',
          priority: 'medium',
          ...
          category: 'Patient Management',
          description: 'Secondary record for self-service patient access portal'
        },
        {
          id: 'patientportal-003',
          name: 'Sample Patient Portal Record 3',
          status: 'completed',
          priority: 'low',
          ...
          category: 'Patient Management',
          description: 'Completed self-service patient access portal'
        }
      ];
  ...
  const handleView = (item: PatientPortalData) => {
    // Implementation for viewing item details
    console.log('Viewing item:', item);
  };

  const handleEdit = (item: PatientPortalData) => {
    setEditingItem(item);
    setShowModal(true);
  };
  ...
  const handleSave = (formData: Partial<PatientPortalData>) => {
    if (editingItem) {
      ...
      // Add new item
      const newItem: PatientPortalData = {
        id: `patientportal-${Date.now()}`,
        name: formData.name || 'New Patient Portal',
        ...
        category: 'Patient Management',
        description: formData.description || 'Self-service patient access portal'
      };
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem' }}>
        <Loading description="Loading Patient Portal..." />
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '2rem' }}>
        ...
        <Column lg={16} md={8} sm={4}>
          <div style={{ marginBottom: '2rem' }}>
            ...
            <p style={{ color: '#6f6f6f', marginTop: '0.5rem' }}>
              Self-service patient access portal
            </p>
          </div>
        </Column>
      </Grid>
        ...
        <Column lg={16} md={8} sm={4}>
          <DataTable
            ...
            render={({ rows, headers, getTableProps, getHeaderProps, getRowProps }) => (
              <TableContainer title="Patient Portal Management">
                <Table {...getTableProps()}>
                  <TableHead>
                    <TableRow>
                      {headers.map(header => (
      ...
      <Modal
        ...
        modalHeading={editingItem ? `Edit ${editingItem.name}` : 'Add New Patient Portal'}
        primaryButtonText="Save"
        secondaryButtonText="Cancel"
        onRequestSubmit={() => {
          // Get form data and save
          const formData = {
            name: 'Updated Patient Portal',
            status: 'active' as const,
            priority: 'medium' as const,
            assignedTo: 'Current User',
            description: 'Updated self-service patient access portal'
          };
          handleSave(formData);
        }}
      >
