...
interface UtilizationAnalyticsData {
  id: string;
  name: string;
...
const UtilizationAnalytics: React.FC = () => {
  const [data, setData] = useState<UtilizationAnalyticsData[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<UtilizationAnalyticsData | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [analytics, setAnalytics] = useState<any>(null);

  // Sample data specific to Utilization Analytics
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      
      // Simulate API call with healthcare-specific data
      const mockData: UtilizationAnalyticsData[] = [
        {
          id: 'utilizationanalytics-001',
          name: 'Sample Utilization Analytics Record 1',
          status: 'active',
          priority: 'high',
        ...
        {
          id: 'utilizationanalytics-002',
          name: 'Sample Utilization Analytics Record 2',
          status: 'pending',
          priority: 'medium',
          ...
          category: 'Healthcare Analytics',
          description: 'Secondary record for healthcare resource utilization analysis'
        },
        {
          id: 'utilizationanalytics-003',
          name: 'Sample Utilization Analytics Record 3',
          status: 'completed',
          priority: 'low',
          ...
          category: 'Healthcare Analytics',
          description: 'Completed healthcare resource utilization analysis'
        }
      ];
  ...
  const handleView = (item: UtilizationAnalyticsData) => {
    // Implementation for viewing item details
    console.log('Viewing item:', item);
  };

  const handleEdit = (item: UtilizationAnalyticsData) => {
    setEditingItem(item);
    setShowModal(true);
  };
  ...
  const handleSave = (formData: Partial<UtilizationAnalyticsData>) => {
    if (editingItem) {
      ...
      // Add new item
      const newItem: UtilizationAnalyticsData = {
        id: `utilizationanalytics-${Date.now()}`,
        name: formData.name || 'New Utilization Analytics',
        ...
        category: 'Healthcare Analytics',
        description: formData.description || 'Healthcare resource utilization analysis'
      };
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem' }}>
        <Loading description="Loading Utilization Analytics..." />
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '2rem' }}>
        ...
        <Column lg={16} md={8} sm={4}>
          <DataTable
            ...
            render={({ rows, headers, getTableProps, getHeaderProps, getRowProps }) => (
              <TableContainer title="Utilization Analytics Management">
                <Table {...getTableProps()}>
                  <TableHead>
                    <TableRow>
                      {headers.map(header => (
      ...
      <Modal
        ...
        modalHeading={editingItem ? `Edit ${editingItem.name}` : 'Add New Utilization Analytics'}
        primaryButtonText="Save"
        secondaryButtonText="Cancel"
        onRequestSubmit={() => {
          // Get form data and save
          const formData = {
            name: 'Updated Utilization Analytics',
            status: 'active' as const,
            priority: 'medium' as const,
            assignedTo: 'Current User',
            description: 'Updated healthcare resource utilization analysis'
          };
          handleSave(formData);
        }}
      >
