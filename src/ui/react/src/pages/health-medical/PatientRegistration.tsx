...
const PatientRegistration: React.FC = () => {
  ...
  const [editingItem, setEditingItem] = useState<PatientRegistrationData | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [analytics, setAnalytics] = useState<any>(null);

  // Sample data specific to Patient Registration
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      
      // Simulate API call with healthcare-specific data
      const mockData: PatientRegistrationData[] = [
        {
          id: 'patientregistration-001',
          name: 'Sample Patient Registration Record 1',
          status: 'active',
          priority: 'high',
        ...
        {
          id: 'patientregistration-002',
          name: 'Sample Patient Registration Record 2',
          status: 'pending',
          priority: 'medium',
          ...
          category: 'Patient Management',
          description: 'Secondary record for complete patient registration and enrollment system'
        },
        {
          id: 'patientregistration-003',
          name: 'Sample Patient Registration Record 3',
          status: 'completed',
          priority: 'low',
          ...
          category: 'Patient Management',
          description: 'Completed complete patient registration and enrollment system'
        }
      ];
  ...
  const handleView = (item: PatientRegistrationData) => {
    // Implementation for viewing item details
    console.log('Viewing item:', item);
  };

  const handleEdit = (item: PatientRegistrationData) => {
    setEditingItem(item);
    setShowModal(true);
  };
  ...
  const handleSave = (formData: Partial<PatientRegistrationData>) => {
    if (editingItem) {
      ...
      // Add new item
      const newItem: PatientRegistrationData = {
        id: `patientregistration-${Date.now()}`,
        name: formData.name || 'New Patient Registration',
        ...
        category: 'Patient Management',
        description: formData.description || 'Complete patient registration and enrollment system'
      };
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem' }}>
        <Loading description="Loading Patient Registration..." />
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '2rem' }}>
        ...
        <Column lg={16} md={8} sm={4}>
          <div style={{ marginBottom: '2rem' }}>
            ...
            <p style={{ color: '#6f6f6f', marginTop: '0.5rem' }}>
              Complete patient registration and enrollment system
            </p>
          </div>
        </Column>
      </Grid>
        ...
        <Column lg={16} md={8} sm={4}>
          <DataTable
            ...
            render={({ rows, headers, getTableProps, getHeaderProps, getRowProps }) => (
              <TableContainer title="Patient Registration Management">
                <Table {...getTableProps()}>
                  <TableHead>
                    <TableRow>
                      {headers.map(header => (
      ...
      <Modal
        ...
        modalHeading={editingItem ? `Edit ${editingItem.name}` : 'Add New Patient Registration'}
        primaryButtonText="Save"
        secondaryButtonText="Cancel"
        onRequestSubmit={() => {
          // Get form data and save
          const formData = {
            name: 'Updated Patient Registration',
            status: 'active' as const,
            priority: 'medium' as const,
            assignedTo: 'Current User',
            description: 'Updated complete patient registration and enrollment system'
          };
          handleSave(formData);
        }}
      >
