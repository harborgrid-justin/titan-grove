...
const OperationalExcellenceCenter: React.FC = () => {
  ...
  const [metrics, setMetrics] = useState([
    ...
    { id: 'm-3', name: 'Process Cycle Time', value: 78, target: 85, status: 'At Risk' }
  ]);
  ...
  return (
    <div style={{ display: 'flex', height: '100vh' }}>
      ...
      <div style={{ flex: 1, padding: '2rem', backgroundColor: '#f4f4f4', overflowY: 'auto' }}>
        <Grid>
          <Column lg={16}><h1 style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Trophy size={32} />Operational Excellence Center</h1></Column>
          ...
          <Column lg={4} md={4} sm={4}><KPIWidget title="At Risk" value="1" trend="down" trendValue="10%" icon={<ChartLine size={24} />} /></Column>
          ...
          <Column lg={16} style={{ marginTop: '2rem' }}>
            <Tile style={{ padding: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}><h3>Excellence Metrics</h3><Button kind="primary" renderIcon={Add}>Add Metric</Button></div>
              <DataTable rows={[]} headers={[]}>
                {() => (
                  <TableContainer>
                    <Table>
                      <TableHead><TableRow><TableHeader>Metric</TableHeader><TableHeader>Current</TableHeader><TableHeader>Target</TableHeader><TableHeader>Status</TableHeader></TableRow></TableHead>
                      <TableBody>
                        {metrics.map(m => (
                          <TableRow key={m.id}>
