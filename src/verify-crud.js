...
function checkModuleStructure() {
  ...
  modules.forEach(moduleName => {
    ...
    if (fs.existsSync(indexPath)) {
      ...
      const hasDataAccess = indexContent.includes("export * from './data-access'") || 
                            indexContent.includes('data-access/repositories');
      
      // Check for manager class
      const hasManager = indexContent.includes('extends BaseManager') || 
                        indexContent.includes('class.*Manager');
  ...
  console.log(`   With Data Access: ${modulesWithDataAccess} (${Math.round(modulesWithDataAccess/totalModules*100)}%)`);
  console.log(`   With Managers: ${modulesWithManagers} (${Math.round(modulesWithManagers/totalModules*100)}%)`);
