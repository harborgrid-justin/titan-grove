...
import { QueueMetrics, QueueAlert, MessageQueueConfig } from './types';
...
export class QueueMonitor extends EventEmitter {
  ...
  recordMetrics(queueName: string, metrics: QueueMetrics): void {
    ...
    this.recordMetric(
      queueName,
      'processing_time',
      metrics.throughput.averageProcessingTime,
      timestamp
    );
  ...
  private recordMetric(
    ...
    const cutoff = new Date(
      timestamp.getTime() - this.config.retention.metricsHours * 60 * 60 * 1000
    );
  ...
  private calculateQueueHealthScore(queueName: string): number {
    ...
    if (recentMetrics.waiting.length > 0) {
      const avgWaiting =
        recentMetrics.waiting.reduce((sum, m) => sum + m.value, 0) / recentMetrics.waiting.length;
    ...
    if (recentMetrics.processing_time.length > 0) {
      const avgProcessingTime =
        recentMetrics.processing_time.reduce((sum, m) => sum + m.value, 0) /
        recentMetrics.processing_time.length;
      if (avgProcessingTime > this.config.alertThresholds.processingTime) {
        score -= Math.min(
          20,
          (avgProcessingTime / this.config.alertThresholds.processingTime) * 10
        );
      }
    }
    ...
    if (recentMetrics.memory_usage.length > 0) {
      const avgMemoryUsage =
        recentMetrics.memory_usage.reduce((sum, m) => sum + m.value, 0) /
        recentMetrics.memory_usage.length;
  ...
  private calculateSystemMetrics(): HealthStatus['systemMetrics'] {
    ...
    const errorRate =
      totalCompleted + totalFailed > 0 ? totalFailed / (totalCompleted + totalFailed) : 0;
  ...
  private checkAlertConditions(queueName: string, metrics: QueueMetrics): void {
    ...
    if (metrics.waiting > this.config.alertThresholds.queueDepth) {
      this.createAlert({
        ...
        severity:
          metrics.waiting > this.config.alertThresholds.queueDepth * 2 ? 'CRITICAL' : 'HIGH',
  ...
  private cleanupOldMetrics(): void {
    ...
    const alertCutoff = new Date(
      Date.now() - this.config.retention.alertsDays * 24 * 60 * 60 * 1000
    );
  ...
  private exportCSV(): string {
    ...
    for (const [key, metrics] of this.metrics) {
      const [queueName, metricType] = key.split(':');
      for (const metric of metrics.slice(-1000)) {
