...
export abstract class BaseRecommendationGenerator<T extends BaseRecommendationCriteria> {
  ...
  protected abstract getUnderperformanceRecommendation(
    ratio: number,
    context: RecommendationContext
  ): string;
  ...
  protected prioritizeRecommendations<T extends { priority: Priority }>(recommendations: T[]): T[] {
    const priorityOrder = {
      [Priority.CRITICAL]: 0,
      [Priority.HIGH]: 1,
      [Priority.MEDIUM]: 2,
    ...
    if (variance > varianceThreshold) {
      if (actualCost > budgetedCost) {
        ...
        recommendations.push(
          'Review spending patterns and identify cost optimization opportunities'
        );
      } else {
        recommendations.push(
          'Analyze underspend to identify potential budget reallocation opportunities'
        );
      }
    }
