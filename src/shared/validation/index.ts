...
export const BusinessRules = {
  ...
  validateProjectBudget: (budget: number, actualCost: number = 0): { valid: boolean; message?: string } => {
    if (!ValidationUtils.isValidAmount(budget)) {
      return { valid: false, message: ErrorMessages.INVALID_AMOUNT('Budget') };
    }
    if (actualCost > budget * 1.2) { // 20% over budget threshold
  ...
  validateProfitMargin: (revenue: number, costs: number): { valid: boolean; message?: string; margin: number } => {
